...
def dump_unet(sd, out_dir, cfg):
    ...
    for i in range(n_down):
        ...
        dump_res_block(arena, sd, f"{p}.0")
        dump_res_block(arena, sd, f"{p}.1")
        if f"{p}.2.weight" in sd:                      # Conv1d, else Identity
            conv1d_rows(arena, sd[f"{p}.2.weight"], sd[f"{p}.2.bias"])

    dump_res_block(arena, sd, "diffusion.unet.mid_modules.0")
    dump_res_block(arena, sd, "diffusion.unet.mid_modules.1")
...
# stats + meta
# ---------------------------------------------------------------------------
def dump_stats(stats, cam_keys, out_dir):
    ...
    def get(key, field):
        ...
        return np.asarray(to_numpy(s[field]), dtype=np.float32).reshape(-1)

    parts = [get("observation.state", "min"), get("observation.state", "max"),
             get("action", "min"), get("action", "max")]
    for k in cam_keys:
        s = stats.get(k, {})
        mean = np.asarray(to_numpy(s["mean"]), np.float32).reshape(-1) if "mean" in s \
            else np.zeros(3, np.float32)
        std = np.asarray(to_numpy(s["std"]), np.float32).reshape(-1) if "std" in s \
...
def dump_meta(cfg, cam_names, img_hw, out_dir, scheduler, steps):
    ...
    write_meta(out_dir, "diffusion", lines)
