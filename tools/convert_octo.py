...
Convert the authoritative Octo-Small-1.5 JAX checkpoint into the vla.simd GGUF
the engine loads. Setup env according to README.md, then run at the
top of the repo:

  python tools/convert_octo.py [OUT] [--ckpt ID_OR_DIR] [--inspect]
...
Packed into one GGUF (default build/octo/octo.gguf; tools/_gguf.py), as these files:
  ...
  tok/vocab.txt              the t5-base sentencepiece vocabulary
"""
import argparse
import os
import sys
...
_p.add_argument("out", nargs="?", default="build/octo",
                help="output .gguf, or a dir to write <dir>/<dir>.gguf in (default: build/octo)")
...
with gguf_output(_a.out, "octo", source=CKPT) as _o:
    OUT = _o.dir
    ...
    from convert_t5_tokenizer import export
    export(f"{OUT}/tok")
