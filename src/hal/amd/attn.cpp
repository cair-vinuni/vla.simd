...
// AMD Zen masked attention. Same math as the Intel AVX2 backend - same QK tile,
// same Cephes exp, same accumulation order, bit-identical outputs - with two
// memory-side changes that the Intel L1 hides and Zen 3's does not:
//
//   1. K^T stride padding (hal/common/layout.h). The QK tile strides ldk floats
//      down 64 head-dim rows; at seq 1024 the natural 4 KB stride aliases all 64
//      rows onto one 8-way L1 set. Padding by a vector: QK 389 -> 528 GF/s.
//
//   2. Tiled A*V. The Intel kernel finishes one query row at a time, so every
//      row re-streams the head's whole V slice (256 KB at seq 1024) - 6x the
//      traffic of the QK tile that produced its scores, and A*V was 62% of the
//      op. Here the 6 rows of a QK tile share every V load: 6 rows x 16 output
//      lanes = 12 accumulators + 2 V loads + 1 broadcast = 15 YMM, the same
//      register budget as the QK and GEMM tiles, walked in 4 passes over
//      head_dim. V bytes per query row drop 6x (256 B -> 42.7 B per key).
//
// Bit-exactness vs the per-row form (the property that lets this backend reuse
// the Intel goldens):
...
//
// V comes pre-packed as [kv][d/16][key][16] (vpack below), so each of the four
// head_dim passes streams one contiguous run instead of walking the model's
// [key][kv][d] layout at an n_kv*head_dim stride. That stride is the same L1/L2
// set-aliasing trap as the K^T one: at the ViT shape it is 3072 B, which folds
// the head's 256 KB V slice onto 64 of the L2's 1024 sets (16 lines deep in an
// 8-way cache), so a "resident" slice re-misses every pass. Measured A*V alone
// at seq 1024, 6 threads: 411 -> 676 GF/s, 1.49x including the pack.
template <int ROWS>
static inline void av_tile16(const float* p, size_t lds, float* o, size_t ldo,
                             const float* VP, int av_end, int seq_k, int kv,
                             int head_dim) {
    for (int d0=0; d0<head_dim; d0 += 16) {
        ...
        const float* vv = VP+((size_t)kv*(head_dim/16)+d0/16)*(size_t)seq_k*16;
        for (int t2=0; t2<av_end; t2++, vv += 16) {
...
static inline void av_tile16_rows(int rows, const float* p, size_t lds, float* o, size_t ldo,
                                  const float* VP, int av_end, int seq_k, int kv, int head_dim) {
    switch (rows) {
        case 6: av_tile16<6>(p, lds, o, ldo, VP, av_end, seq_k, kv, head_dim); break;
        case 5: av_tile16<5>(p, lds, o, ldo, VP, av_end, seq_k, kv, head_dim); break;
        case 4: av_tile16<4>(p, lds, o, ldo, VP, av_end, seq_k, kv, head_dim); break;
        case 3: av_tile16<3>(p, lds, o, ldo, VP, av_end, seq_k, kv, head_dim); break;
        case 2: av_tile16<2>(p, lds, o, ldo, VP, av_end, seq_k, kv, head_dim); break;
        default: av_tile16<1>(p, lds, o, ldo, VP, av_end, seq_k, kv, head_dim); break;
    }
}
...
// ROWSx16 QK micro-kernel over transposed keys (identical to the Intel one; the
// stride it walks is the padded kt_stride).
template <int ROWS>
...
void gqa_attention_masked(float* out, const float* Q, const float* K, const float* V,
    ...
    const int sds   = ((seq_k+7) & ~7) + 8;    // scores leading dimension (same aliasing)
    ...
    if (!KTp) {
        ...
        for (int jb=0; jb<skp; jb += 8) {
            ...
            const int je  = rem < 0 ? 0 : (rem < 8 ? rem : 8);   // keys present in this group
            const int jz  = skp-jb < 8 ? skp-jb : 8;             // lanes this group owns
            for (int kv=0; kv<n_kv; kv++)
                for (int d=0; d<head_dim; d++) {
    ...
    if (hal::env::attn_qtile() && seq_q >= 6 && head_dim%16 == 0) {
        // Query-tiled: 6 rows share the K^T streams (QK) and the V streams (A*V).
        // The row bound is hoisted; a tile softmaxes over the tile-wide bound, so
        // the shared A*V can run one key loop for all its rows.
        constexpr int QR = 6;
        ...
        float* const VP = VPbuf.data();
        vpack(VP, V, seq_k, n_kv, head_dim);
...
#endif
      {
        ...
        for (int u=0; u<n_q*ntiles; u++) {
            ...
            if (all_ok) {
                av_tile16_rows(rows, scores.data(), sds, o0, ldq, VP, tjm, seq_k, kv, head_dim);
                continue;
            }
